...
final class RetryAfterParsingTests: XCTestCase {
    ...
    func test_givenSecondsHeader_whenParsing_thenReturnsThoseSeconds() throws {
        XCTAssertEqual(URLSessionNetworkService.retryAfter(from: try response(retryAfter: "30")), 30)
    }

    func test_givenNoHeader_whenParsing_thenReturnsNil() throws {
        XCTAssertNil(URLSessionNetworkService.retryAfter(from: try response(retryAfter: nil)))
    }

    func test_givenGarbageHeader_whenParsing_thenReturnsNil() throws {
        XCTAssertNil(URLSessionNetworkService.retryAfter(from: try response(retryAfter: "soon")))
    }
