import SwiftUI

/// The app's own nameplate, replacing the stock inline navigation title.
///
/// The navigation title is why the top of the feed read as a generic application. A tracked
/// wordmark over a hairline is the first thing that says this is a paper.
///
/// The search affordance is optional and only supplied where it opens something, which is why it
/// was absent until ticket 33 built the screen behind it.
struct MastheadView: View {
    let title: String
    var searchLabel: String?
    var onSearch: (() -> Void)?

    var body: some View {
        ZStack {
            Text(title.uppercased())
                // A step up from the metadata size the rest of the app uses for small tracked
                // text. The nameplate is the one place where being slightly louder is the point.
                .font(Tokens.Typography.masthead)
                .tracking(2.6)
                .foregroundStyle(Tokens.Palette.ink)
                .lineLimit(1)
                .accessibilityAddTraits(.isHeader)

            if let searchLabel, let onSearch {
                // Overlaid rather than placed in a row, so the wordmark stays centred on the
                // screen rather than centred on what is left beside the icon.
                HStack {
                    Spacer()
                    Button(action: onSearch) {
                        Image(systemName: "magnifyingglass")
                            .font(Tokens.Typography.category.weight(.semibold))
                            .foregroundStyle(Tokens.Palette.ink)
                            .frame(width: 44, height: 44)
                            .contentShape(Rectangle())
                    }
                    .buttonStyle(.plain)
                    .accessibilityLabel(searchLabel)
                    .accessibilityIdentifier("masthead.search")
                }
                .padding(.trailing, Tokens.Space.s)
            }
        }
            .frame(maxWidth: .infinity)
            .frame(height: 44)
            .background(Tokens.Palette.background)
            .overlay(alignment: .bottom) {
                Rectangle()
                    .fill(Tokens.Palette.hairline)
                    .frame(height: 1)
            }
            // The nameplate is fixed furniture. Letting it grow with text pushes the news off
            // the screen to say something the reader already knows.
            .dynamicTypeSize(...DynamicTypeSize.large)
    }
}
